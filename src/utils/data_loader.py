...
class DataLoader:
    ...
    def enriquecer_datos(self, df):
        """Enriquece datos con variables macro VARIABLES por mes"""
        df_enriquecido = df.copy()

        # ✅ MEJORADO: Generar variación realista mensual
        n_meses = len(df_enriquecido)

        # UVR con variación mensual pequeña (~1-2%)
        uvr_base = self.valores_actuales.get("tasa_uvr", 12.0)
        df_enriquecido["tasa_uvr"] = uvr_base + np.random.normal(0, 0.3, n_meses)

        # DTF con variación mensual
        dtf_base = self.valores_actuales.get("tasa_dtf", 10.5)
        df_enriquecido["tasa_dtf"] = dtf_base + np.random.normal(0, 0.2, n_meses)

        # IPC con variación mensual
        ipc_base = self.valores_actuales.get("inflacion_ipc", 5.5)
        df_enriquecido["inflacion_ipc"] = ipc_base + np.random.normal(0, 0.5, n_meses)
        ...
        try:
            ...
            if not isinstance(df_base.index, pd.DatetimeIndex):
                if "fecha" in df_base.columns:
                    ...
                    df_base.index = pd.date_range(
                        start="2025-01-01", periods=len(df_base), freq="M"
                    )
